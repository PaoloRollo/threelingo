...
export default function Leaderboard() {
  ...
  return (
    ...
    >
      <h1 className="font-bold text-2xl tracking-wide mb-4">leaderboard</h1>
        ...
        leaderboard.map((leaderboardEntry, index) => {
          return (
            <LeaderboardProfile
              key={index}
              place={index + 1}
              name={sliceAddress(leaderboardEntry.address)}
              xp={leaderboardEntry.xp}
              isCurrentUser={
                address.toLowerCase() === leaderboardEntry.address.toLowerCase()
              }
            ></LeaderboardProfile>
          );
        })}
